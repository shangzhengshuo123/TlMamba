...
class SS_Conv_SSM(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        input_left = self.cf_block(input_left)
...
class VSSM(nn.Module):
    ...
    def forward_features(self, x):
        """
        提取中间特征用于可视化或其他下游任务（如 t-SNE）。
        返回的是 avgpool 之前的高维表示。
        """
        x = self.forward_backbone(x)  # 输出为 (B, H, W, C)
...
tlmamba_t = VSSM(depths=[2, 2, 4, 2], dims=[96, 192, 384, 768], num_classes=6).to("cuda")
tlmamba_s = VSSM(depths=[2, 2, 8, 2], dims=[96, 192, 384, 768], num_classes=6).to("cuda")
tlmamba_b = VSSM(depths=[2, 2, 12, 2], dims=[128, 256, 512, 1024], num_classes=6).to("cuda")

data = torch.randn(1, 3, 224, 224).to("cuda")

print(tlmamba_t(data).shape)