...
try:
    ...
    # Fallbacks keep this module usable when mmengine is unavailable.
    def constant_init(module_or_param, val=0, bias=None):
        target = module_or_param.weight if hasattr(module_or_param, "weight") else module_or_param
        nn.init.constant_(target, val)
        if bias is not None and hasattr(module_or_param, "bias") and module_or_param.bias is not None:
            nn.init.constant_(module_or_param.bias, bias)

    def kaiming_init(module_or_param, **kwargs):
        target = module_or_param.weight if hasattr(module_or_param, "weight") else module_or_param
        nn.init.kaiming_normal_(target, mode="fan_out", nonlinearity="relu")

    def trunc_normal_init(module_or_param, std=.02, mean=0., **kwargs):
        target = module_or_param.weight if hasattr(module_or_param, "weight") else module_or_param
        nn.init.trunc_normal_(target, mean=mean, std=std)

    def normal_init(module_or_param, mean=0., std=1., **kwargs):
        target = module_or_param.weight if hasattr(module_or_param, "weight") else module_or_param
        nn.init.normal_(target, mean=mean, std=std)
